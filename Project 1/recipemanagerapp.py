...
menu = st.sidebar.radio("Choose an option:",
                         [ "➕ Add New Recipe", "🕵🏼 Search by Ingredient", "📖 View All Recipes", "🎲 Random Recipe","🔎 Browse by Category","⭐ Rate a Recipe","🛒 Shopping List","✔️ Cooking History","🌐 Search Online","🤖 Ask the AI Chef"])
...
elif menu=="🌐 Search Online":
    st.header("🌐 Search Online")
    st.write("Search for new recipe ideas from an online recipe API.")

    dish=st.text_input("Search for a dish:")

    if st.button("Search Online"):
        if dish.strip()=='':
            st.warning("⛔ Please enter a dish name.")
        else:
            meals=search_online(dish)

            if not meals:
                st.info("ℹ️ No recipes found online.")
            else:
                st.success(f"Found {len(meals)} recipe/recipes.")
...
elif menu=="🤖 Ask the AI Chef":
    st.header("🤖 Ask the AI Chef")
    st.write("Tell the AI what you have and it will pick a recipe from your collection.")

    ingredients=st.text_area("What do you have in your kitchen?",placeholder="chicken, rice, one lemon, yoghurt")

    if st.button("Suggest a recipe"):
        if ingredients.strip()=='':
            st.warning("⛔ Please enter some ingredients.")
        elif recipes.empty:
            st.warning("⛔ Add a recipe first.")
        else:
            answer=suggest_recipe(recipes,ingredients)
            st.write(answer)
