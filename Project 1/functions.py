...
# Read the .env file so the API keys are available
load_dotenv('.env')
...
def search_online(dish):
    """
    Searches an online recipe API for a dish.

    Parameters:
    dish(str): The dish name to search for

    Returns:
    meals(list): List of recipes found, empty list if none
    """
    url="https://www.themealdb.com/api/json/v1/1/search.php"

    try:
        response=requests.get(url, params={'s':dish}, timeout=10)
...
def get_llm_response(prompt):
    ...
    try:
        ...
        status=getattr(e, "status_code", "no status")
        response=f"Error {status}: {e}"

    return response

def suggest_recipe(data, ingredients):
    """
    Asks the LLM which saved recipe to cook with the ingredients on hand.

    Parameters:
    data(dataframe): All recipes
    ingredients(str): What the user has in the kitchen

    Returns:
    response(str): The LLM's suggestion
    """
    collection=data[['name','ingredients']].to_string(index=False)
    prompt=f"My recipes:\n{collection}\n\nI have: {ingredients}\nWhich recipe should I cook and what do I still need to buy?"
