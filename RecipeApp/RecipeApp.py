...
menu = st.sidebar.radio("Choose an option:",
                         [ "➕ Add New Recipe", "🕵🏼 Search by Ingredient", "📖 View All Recipes", "🎲 Random Recipe","🔎 Browse by Category","⭐ Rate a Recipe","✔️ Cooking History","🛒Shopping List"])
...
if menu == "➕ Add New Recipe":
    ...
    if st.button("Add Recipe"):
        if name.strip()=='' or ingredients.strip()=='' or instructions.strip()=='':
            ...
            save_recipes(recipes,"recipes.csv")
...
# RANDOM RECIPE with scalling     
    ...
    if 'random_recipe' in st.session_state:
        ...
        scaled_ingredients=scale_recipe(recipe['ingredients'],recipe['servings'], desired_servings)
        st.write(f"Scaled Ingredients: {scaled_ingredients}")

# Browse by Category (additional sidebar)
